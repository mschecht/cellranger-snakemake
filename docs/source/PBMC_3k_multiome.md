...
To enable, add `jobmode` and related fields to `cellranger_arc` in your config:
...
> 📌 **Note**: When `jobmode` is set to `slurm`, the `threads` and `mem_gb` fields apply to the lightweight Snakemake wrapper job only — not to Cell Ranger ARC's internal compute. Reduce them to `threads: 1` and `mem_gb: 8`. Cell Ranger ARC sizes its own subjobs using `mempercore`.
